...
void card(TFT_eSPI&t,int x,const char*label,Risk r){uint16_t c=color(r);t.fillRoundRect(x,104,96,48,4,COL_INPUTBG);t.drawRoundRect(x,104,96,48,4,g_themeColor);t.setTextFont(FONT_SM);t.setTextColor(g_themeColor,COL_INPUTBG);t.setCursor(x+7,111);t.print(label);t.setTextFont(FONT_MD);t.setTextColor(c,COL_INPUTBG);t.setCursor(x+7,128);t.print(name(r));}
}
void screenFireteamDraw(TFT_eSPI &tft, bool wifiOk) {
    char ts[10];timeGetShort(ts);drawTopbar(tft,g_location.valid?g_location.city:"","FIRETEAM",ts,wifiOk);drawBottombar(tft,"TACTICAL CONDITIONS",4,11);tft.fillRect(0,CONTENT_Y,SCREEN_W,CONTENT_H,COL_BG);
